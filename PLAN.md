...
2. **Brand imagery:** published photos live under `/images/gda/` (advisory meeting, office meeting, seminar, collaborative meeting, strategy focus; logos as `logo-*.png`). The folder `glorious_destiny_mockups/` is gitignored (avoid duplicate large assets); keep originals locally if you need source files.
3. **Wire forms**
